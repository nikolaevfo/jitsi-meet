...
import { checkIsViolation } from "./propagation";
...
export function loadWorker() {
    return function (dispatch: Function) {
        ...
        let baseUrl = "";
        const app: Object = document.querySelector(
    ...
    return async function (dispatch: Function, getState: Function) {
        ...
        Promise.all([
            ...
            setInterval(async () => {
                ...
                if (localVideoTrackPropagation === undefined) {
                    return;
                }

                const streamPropagation =
                    localVideoTrackPropagation.jitsiTrack.getOriginalStream();

                if (streamPropagation === null) {
                    return;
                }
                ...
                if (checkIsViolation(detections)) {
                    ...
                    isViolation = false;
                    isBadViolation = false;
                }

                logger.log(`checkIsViolation ${checkIsViolation(detections)}`);
            }, 1000);
        });
...
function addFacialExpression(
    ...
    return function (dispatch: Function, getState: Function) {
        const { detectionTimeInterval } =
            getState()["features/facial-recognition"];
