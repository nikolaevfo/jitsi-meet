 ...
 */
const AbstractPollAnswer =
    (Component: AbstractComponent<AbstractProps>) => (props: InputProps) => {
        const { pollId } = props;

        const conference: Object = useSelector(
            (state) => state["features/base/conference"].conference
        );

        const poll: Poll = useSelector(
            (state) => state["features/polls"].polls[pollId]
        );
