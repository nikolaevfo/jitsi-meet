...
const SpeakerStats = () => {
    const { enableDisplayFacialExpressions } = useSelector(
        (state) => state["features/base/config"]
    );
    const { showFacialExpressions } = useSelector(
        (state) => state["features/speaker-stats"]
    );
    const { clientWidth } = useSelector(
        (state) => state["features/base/responsive-ui"]
    );
    const displaySwitch =
        enableDisplayFacialExpressions &&
    ...
    return (
        <Dialog
            ...
            width={showFacialExpressions ? "500px" : "small"}
        >
            <div className="speaker-stats">
                <div
                    className={`${classes.searchSwitchContainer}
                        ${
                            showFacialExpressions
                                ? classes.searchSwitchContainerExpressionsOn
                                : ""
                        }`}
                >
                    <div
